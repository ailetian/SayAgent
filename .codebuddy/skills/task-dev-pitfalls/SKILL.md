...
description: 在 hify_python 仓库按子任务（Mx/Ty）做 Java 开发、写单测、写 Flyway 迁移/DDL、处理鉴权与秘钥隔离时加载本 skill。它只沉淀 CLAUDE.md 未覆盖的实战坑与项目特定陷阱（write_to_file 偶发写空、包路径错配、ErrorCode 复用、Retry 语义、秘钥脱敏、软删约定等），并自动把新坑回写本 skill 持续进化。规范原文以 CLAUDE.md 为准，本 skill 不重复。
---

# 任务开发实战坑位（hify_python）

本 skill 只记 **CLAUDE.md 未覆盖** 的踩坑与项目特定陷阱，做到"同样的坑只踩一次"。规范细节（命名/脱敏/模板）以 CLAUDE.md 原文为准，不在此复述。
